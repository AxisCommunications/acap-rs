...
fn generate_license(out_dir: &Path) {
    let license_in = "build/LICENSE";
    let license_out = out_dir.join("LICENSE");
    let content = fs::read_to_string(license_in).unwrap();
    println!("cargo:rerun-if-changed={license_in}");
    fs::write(license_out, content).unwrap();
}
