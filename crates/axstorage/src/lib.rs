...
pub fn subscribe<F>(storage_id: &mut GStringPtr, callback: F) -> Result<SubscriptionId, glib::Error>
where
    F: FnMut(&GStringPtr, Option<glib::Error>) + Send + 'static,
{
    ...
    unsafe {
        ...
        // TODO: Is it safe to move the `Box` here considering
...
pub fn unsubscribe(id: &SubscriptionId) -> Result<(), glib::Error> {
    // TODO: SAFETY
    unsafe {
        let success = try_func!(ax_storage_unsubscribe, id.id);
