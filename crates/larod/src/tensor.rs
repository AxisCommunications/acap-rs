    ...
    fn drop(&mut self) {
        ...
        let (success, maybe_error) = crate::with_larod(|| {
            ...
            let maybe_error = if error.is_null() {
                ...
                // SAFETY: error was allocated by larodDestroyTensors and the
...
impl<'a> TensorRef<'a> {
    ...
    /// Returns the tensor's file descriptor.
    ///
    /// larod reports `LAROD_INVALID_FD` when the call fails, including when no
    /// fd has been assigned to the tensor yet, so those cases surface as an
    /// error rather than a separate "unset" value.
    ///
    /// # Safety
    ///
    ...
    pub unsafe fn fd(&self) -> Result<c_int, Error> {
        let (fd, maybe_error) =
...
fn destroy_tensors(
    ...
    let (success, maybe_error) = crate::with_larod(|| {
        let mut error: *mut larod_sys::larodError = std::ptr::null_mut();
        let success =
            unsafe { larod_sys::larodDestroyTensors(conn_raw, &mut raw, len, &mut error) };
        let maybe_error = if error.is_null() {
            None
        } else {
            // SAFETY: error was allocated by larodDestroyTensors and the
...
/// Decode the return value of `larodGetTensorFd`.
///
/// `LAROD_INVALID_FD` (`INT_MIN`) is the documented failure sentinel; larod
/// normally populates an error object alongside it. Any other negative value
/// is not a valid fd and is reported as such.
fn decode_fd(fd: c_int, maybe_error: Option<Error>) -> Result<c_int, Error> {
    if let Some(err) = maybe_error {
        return Err(err);
    }
    if fd == c_int::MIN {
...
mod tests {
    ...
    #[test]
    fn invalid_fd_sentinel_with_error_reports_that_error() {
        assert!(matches!(
            super::decode_fd(c_int::MIN, Some(Error::NullPointer)),
    ...
    fn non_negative_fd_is_valid() {
        assert!(matches!(super::decode_fd(0, None), Ok(0)));
    }
}
